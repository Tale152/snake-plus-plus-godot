class_name DiamondStrategy extends CollisionStrategy

const _DIAMOND_SCORE = 10000
...
func execute(
	...
	if snake_properties.is_intangible():
		...
		snake_properties.set_score(
			snake_properties.get_score() + (snake_properties.get_score_multiplier() * _DIAMOND_SCORE)
		)

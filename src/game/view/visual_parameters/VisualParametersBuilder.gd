...
func add_perk_sprite(sprite: PerkSprite) -> VisualParametersBuilder:
	_perks_sprites.push_back(sprite)
	return self
...
func build() -> VisualParameters:
	...
	_scale_array_of_sprites(_perks_sprites, _cell_pixels_size)
...
func _populate_snake_sprites(head_sprites, body_sprites, tail_sprites) -> void:
	for d in Direction.get_directions():
		...
		for j in Direction.get_directions():
			if d != j:
...
func _populate_walls_sprites(walls_sprites: Array) -> void:
	for u in 2: for r in 2: for d in 2: for l in 2:
		var connections = []
		if u == 1: connections.push_back(Direction.UP())
		if r == 1: connections.push_back(Direction.RIGHT())
		if d == 1: connections.push_back(Direction.DOWN())
		if l == 1: connections.push_back(Direction.LEFT())

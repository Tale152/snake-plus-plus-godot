...
func initialize(
	...
	_possible_effects = effects.duplicate(false)
	_real_heigth_px = _DEFAULT_HEIGHT_PX * scale
	_real_width_px = _DEFAULT_WIDTH_PX * scale
	for e in effects:
		var es = EffectStatus.instance()
		es.initialize(visual_parameters.get_edible_sprite(e), _real_heigth_px)
	...
	if equipped_effects_timers.size() > 0:
		...
		for timer in equipped_effects_timers:
			...
			if !_currently_printed_effects.has(type):
				es = _possible_effect_status[type]
			...
			es.render(effect_margins[0], effect_margins[1], _get_timer_percentage(timer))
			i += 1
...
func _get_timer_percentage(t: EquippedEffectTimer) -> int:
	return int(floor(100 - (t.get_elapsed_time() * 100 / t.get_total_time())))

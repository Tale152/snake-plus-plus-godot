...
func get_field_size() -> FieldSize:
	return FieldSize.new(int(_stage.field.size[0]), int(_stage.field.size[1]))

func get_walls_points() -> Array:
	var res: Array = []
	for wall in _stage.field.walls:
		res.push_back(ImmutablePoint.new(int(wall[0]), int(wall[1])))
	return res

func get_snake_spawn_point() -> ImmutablePoint:
	return ImmutablePoint.new(int(_stage.snake.spawn[0]), int(_stage.snake.spawn[1]))
